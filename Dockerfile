...
LABEL description="Kubevirt Manager 1.3.2 - Builder"

WORKDIR /usr/src/app
COPY . /usr/src/app
RUN cd /usr/src/app/src/assets/ && git clone https://github.com/novnc/noVNC.git
RUN cd /usr/src/app && npm run clean && npm install -g @angular/cli@16.2.12 && npm install && npm run build

# NGINX Image
FROM nginx:1.25-alpine

LABEL org.opencontainers.image.authors="marcelo@feitoza.com.br"
LABEL description="Kubevirt Manager 1.3.2"
